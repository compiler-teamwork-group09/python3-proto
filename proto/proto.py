    ...
    if cls is not Proto:
        if not skip and not mo:
            ...
            if result is not None:
                if bound is not None and isinstance(result, Method):
                    yield result.method(bound)
                else:
                    yield result if not hasattr(result, '__get__') else result.__get__(cls, cls.__class__)
...
class Proto(type):
    ...
    def __new__(mcs, *args, **kwargs):
        ...
        try:
            ...
            name = 'Anonymous Proto'
        try:
            sup = tuple(kwargs.pop('__super__'))
        except KeyError:
            sup = ()
    ...
    def __init__(cls, *args, **kwargs):
        ...
        """
        if super(Proto, cls).__init__(cls, cls) is not Proto:
            try:
    ...
    def getitem(mcs):
        ...
        def __wrapper__(f):
            mcs.__proto_getitem__ = Method(f)
    ...
    def setitem(mcs):
        ...
        def __wrapper__(f):
            mcs.__proto_setitem__ = Method(f)
    ...
    def __enter__(mcs):
        ...
        """
        try:
            user_def_enter = mcs.super_method_only(mcs, '__proto_enter__')
        except AttributeError:
            mcs.__proto_previous_locals__ = inspect.currentframe().f_back.f_locals.copy()
            return mcs
        user_def_enter()

    @classmethod
    def __exit__(mcs, exc_type, exc_val, exc_tb):
        """ default exit protocol, which set the attributes that newly defined with the block
        :return: None
        """
        try:
            user_def_exit = mcs.super_method_only(mcs, '__proto_enter__')
        except AttributeError:
            previous_locals = mcs.__proto_previous_locals__
            for key, val in inspect.currentframe().f_back.f_locals.items():
                if key not in previous_locals or previous_locals[key] not in (val, mcs):
                    if isinstance(val, _function):
                        setattr(mcs, key, Method(val))
                    elif not isinstance(val, Method):
                        setattr(mcs, key, val)
                    elif isinstance(val, ClassMethod):
                        setattr(mcs, key, val.method(mcs))
                    else:
                        setattr(mcs, key, val.func)
            return
    ...
    def __getitem__(mcs, item):
        """ hook redirect
        """
        return mcs.super_method_only(mcs, '__proto_getitem__')(item)

    @classmethod
    def __setitem__(mcs, key, value):
        """ hook redirect
        """
        return mcs.super_method_only(mcs, '__proto_setitem__')(key, value)
